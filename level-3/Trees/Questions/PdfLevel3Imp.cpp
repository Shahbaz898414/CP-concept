...
void dfs(int curr, vector<vector<int>> &edgs, int currParent, vector<vector<int>> &parents)
{
...
int lca(int x,int y,vector<vector<int>> &parent,vector<int>&inTime,vector<int>&outTime) {
  ...
  for (int i = mxlog-1; i >=0; i--) {
    ...
    if(parentofX!=-1 and !isAncentor(parentofX,y,inTime,outTime)){
  ...
  while (t--)
  {
   ...
   int root = 0;
    ...
    while(q--) {
      ...
      cout<<lca(a,b,parents,inTime,outTime)+1<<endl;
