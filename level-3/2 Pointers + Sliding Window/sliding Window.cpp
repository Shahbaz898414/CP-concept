...
/*--------------------------------------------------------------------------------------------------------------------------*/
ll range(vector<ll>& prefix, int l, int r){
    return mod_mul(prefix[r], (l > 0 ? mminvprime(prefix[l - 1], MOD) : 1LL), MOD); 
}
...
void solve(){
    ...
    while(t--){
        ...
        vector<pair<ll, ll>> a(all(arr));
        n = sz(a);
        vector<ll> prefix(n, a[0].ss);
        for(int i = 1; i < n; i++){
            prefix[i] = mod_mul(a[i].ss, prefix[i - 1], MOD);
        }
        ll ans = 0;
        for(int i = 0; i <= n - m; i++){
            if(m == 1){
                ans = mod_add(ans, a[i].ss, MOD);
                continue;
            }
            if(a[i + m - 1].ff - a[i].ff < m){
                ans = mod_add(ans, a[i].ss * range(prefix, i + 1, i + m - 1), MOD);
            }
        }
        cout << ans << nline;
    }
